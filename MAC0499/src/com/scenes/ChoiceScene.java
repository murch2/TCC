...
HTTPResponseListener, IOnMenuItemClickListener {
	...
	private void createItensScene(JSONArray dados) {
		createBackground();
//		Se eu num vou escrever o que eh cada cor eu não preciso dos dados.
	...
	private void createRoullete(JSONArray dados) {
		try {
			System.out.println("Json da roleta = " + dados.toString(4));
	...
	// Aqui devolve as informações, devo criar um array de um objeto que contem
	// essas informações, tem que ter um if pra ver se deu tudo ok.
	@Override
	...
	private MenuScene setMenuLayoutToHorizontal(MenuScene menu, int padding) {
		...
		for (int i = 1; i < menu.getChildCount(); i++) {
			System.out.println("Posicao = "
					+ menu.getChildByIndex(i - 1).getX());
			menu.getChildByIndex(i).setPosition(
					menu.getChildByIndex(i - 1).getX()
					+ menu.getChildByIndex(i).getWidth() + padding,
